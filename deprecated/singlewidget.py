...
from deprecated.stepscan import stepscan
...
class StepScanCentralWidget(QtWidgets.QWidget):
    ...
    def set_time_axis(self):
        ...
        print('StepScan contains {0} points, ranging from {1} to {2} ps.'.format(len(self.scan.timeScale),
                                                                                 self.scan.timeScale[0],
                                                                                 self.scan.timeScale[-1]))
    ...
    def startstop_scan(self):
        if self.scanning:
            ...
            for parameter in self.lockinParameters:
                self.scan.data[parameter] = pd.DataFrame(np.zeros(len(self.scan.timeScale)), columns=['avg'],
                                                         index=self.scan.timeScale)
            self.plotData['avg'] = pd.DataFrame(np.zeros(len(self.scan.timeScale)), columns=['avg'],
                                                index=self.scan.timeScale)
            self.scanNumber = 0
    ...
    def make_single_scan(self):
        ...
        self.scan_thread = QtCore.QThread()
        print(
            'scanning for {0} with {1} dwelltime'.format(self.lockinParameters, self.scan.settings_lockIn['dwellTime'],
                                                         self.instruments))
